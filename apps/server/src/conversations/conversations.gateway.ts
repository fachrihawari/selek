...
export class ConversationsGateway implements OnGatewayInit {
  ...
  async handleCome(@ConnectedSocket() socket: Socket, @MessageBody() workspaceId: string) {
    const conversations = await this.conversationsService.getConversations(workspaceId, socket.user.id)

    console.log('Connected user ->', socket.id);
    console.log(socket.user);

    console.log('joining room -> workspaces:' + workspaceId);
    socket.join('workspaces:' + workspaceId)

    for (const c of conversations) {
      socket.join('conversations:' + c.id)
